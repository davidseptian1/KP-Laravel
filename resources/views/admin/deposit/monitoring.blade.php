...
<script>
    // ============================================================
    // Filter Toggle: Bukti Penambahan
    // State: 0 = Semua, 1 = Ada isi (aktif), 2 = Kosong (non-aktif)
    // ============================================================
    (function () {
        let filterState = 0; // 0=semua, 1=ada isi, 2=kosong

        const th = document.getElementById('thBuktiPenambahan');
        const badge = document.getElementById('buktiPenambahanFilterBadge');
        ...
        function applyFilter() {
            ...
            const rows = table.querySelectorAll('tr');
            rows.forEach(function (row) {
                const td = row.querySelector('td[data-has-bukti-penambahan]');
                if (!td) return;

                const hasContent = td.getAttribute('data-has-bukti-penambahan') === '1';
        ...
        function updateThStyle() {
            if (filterState === 0) {
                ...
                badge.style.display = 'none';
                badge.textContent = '';
                th.title = 'Klik untuk filter berdasarkan Bukti Penambahan';
            } else if (filterState === 1) {
                th.style.background = 'var(--bs-success, #198754)';
                th.style.color = '#fff';
                badge.style.display = 'inline-block';
