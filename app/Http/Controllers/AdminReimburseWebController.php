...
class AdminReimburseWebController extends Controller
{
    ...
    private function sendTelegramNotification($reimburse, $oldStatus, $validated)
    {
        ...
        $message = null;
        $isWaitingApproval = ($reimburse->status === 'waiting_approval_direksi');

        // Condition 1: Status is waiting_approval_direksi
        ...
        if ($reimburse->status === 'approved' && !empty($validated['payment_proof_type'])) {
            ...
            $message = "✅ <b>Data Selesai Ditransfer</b>\n\n"
                . "<b>Kode</b>: {$kode}\n"
                . "<b>Nama</b>: {$nama}\n"
                . "<b>Nominal</b>: Rp " . number_format((float)$reimburse->nominal, 0, ',', '.') . "\n"
                . "<b>Status</b>: Sudah Ditransfer";
        }
